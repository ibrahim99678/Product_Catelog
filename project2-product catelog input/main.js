...
function removeItemFromDataStore(products,id) {
...
function removeProductFromStroge(id){
    ...
    //pick from local stroge
    const products = JSON.parse(localStorage.getItem('storeProducts'))
    //filter
    const productAfterRemove = updateAfterRemove(products, id)
    //save data to local stroge
...
function addItemToStroge(){
    ...
    if(localStorage.getItem('storeProducts')) {
        ...
        products.push(product)
        //update to local storge 
        localStorage.setItem('storePructs', JSON.stringify(products))
            ...
            if(!isError) {
                ...
                //add iteam to local storage 
                addItemToStroge(product)
                //console.log(products);
                //reset the input
                resetInput();
            }
    ...
    listGroupElm.addEventListener("click", evt => {
        ...
        if(evt.target.classList.contains("delete-item")) {
            ...
            //delete iteam from storage 
            removeProductFromStroge(id)
    ...
    document.addEventListener('DOMContentLoaded', (e) => {
        //checking iteam into localstorage 
